        ...
        if (idToken != tokens.end()) {
          ...
          if (type->second == "NODE") {
            ...
            if (newNode == _nodes.end()) {
              _nodes.emplace_back(std::make_unique_ptr<>());
          ...
          if (type->second == "EDGE") {
            ...
            if (parentToken != tokens.end() && childToken != tokens.end()) {
              ...
              auto parentNode = std::find_if(
                  ...
                  [&parentToken](std::make_unique_ptr<>()) {
                    return node->GetID() == std::stoi(parentToken->second);
                  });
              auto childNode = std::find_if(
                  _nodes.begin(), _nodes.end(),
                  [&childToken](std::make_unique_ptr<>()) {
                    return node->GetID() == std::stoi(childToken->second);
                  });

              // create new edge
              GraphEdge *edge = std::unique_ptr<GraphEdge>; // todo make it as unique_ptr
